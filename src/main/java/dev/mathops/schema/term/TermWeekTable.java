...
public final class TermWeekTable extends Table {
    ...
    private static final String DESCR = "<p>Each row stores the date ranges for a single \"week\" within a term. "
                                        + "These ranges need not correspond exactly to calendar weeks, but are used " +
                                        "when reporting in which week "
                                        + "a student's work occurred.  The weeks of a term must cover all days " +
                                        "contained in the term, but may not "
                                        + "overlap (every calendar day must uniquely identify a term and a week " +
                                        "number within that term).</p>";

    /** Examples (HTML). */
    private static final String EXAMPLES = "<p>Week 0 could represent any days from the start of the term to the first "
                                           + "day of the first week of class.  Then weeks 1 through 15 would be the " +
                                           "actual weeks of the class (with "
                                           + "each week running from Sunday through Saturday), but week 10 could span" +
                                           " 14 days since it covers "
                                           + "\"Spring Break\".  Then a week 16 could cover any remaining days from " +
    ...
    static {
        F_WEEK_NBR = new Field(new FieldDef("week_nbr", EFieldType.INTEGER,
                "The week number."), EFieldRole.PARTITION_KEY);
        F_START_DATE = new Field(new FieldDef("start_date", EFieldType.LOCAL_DATE,
                "The first day of the week."), EFieldRole.NOT_NULL);
        F_END_DATE = new Field(new FieldDef("end_date", EFieldType.LOCAL_DATE,
                "The last day of the week."), EFieldRole.NOT_NULL);
