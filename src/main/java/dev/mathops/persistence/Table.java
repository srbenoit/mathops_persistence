 ...
 * exist for each defined table, and that class can have utility methods to create criteria or updated value objects
 * for common operations on that table's data.
 *
 * <p>
 ...
 * contexts that require a well-defined order (such as keys in a {@code TreeMap}).  Order is based on schema name
 * and then table name, using {@code String}'s default ordering for each.
 *
 * <p>
 * Typically, concrete subclasses of the {@code Table} class will define their fields, with all required constraints,
 * as class-static instances of {@code Field} with names that are upper-case underscore-separated versions of the
 * field's name.  The field's name itself is often required by specific implementations to map to an actual field name
 * in a database product, so it should limit itself to lowercase ASCII letters, digits, and underscores.  For example,
 * a field with the name "user_id" would typically be defined by a class-static instance of Field named "USER_ID".
 *
 * <p>
 ...
 * </pre>
 *
 * This method should test that the row's owning table matches the single instance of the subclass of {@code Table}
 * that owns the field, but it does not need to test the data type of the value, since it will have been validated on
 * construction.
 */
